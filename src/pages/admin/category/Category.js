    ...
    const deleteItem = (e, categoryId) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                http.delete('/admin/category/' + categoryId).then((response) => response.data)
                    .then((response) => {
                        if (response.status === true) {
                            Swal.fire({
                                position: 'top-end',
                                ...
                                <table className="table table-bordered table-hover">
                                    ...
                                    {categories.map((category, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <Link className="btn btn-sm btn-info mx-1" to={`/admin/category/edit/${category.id}`}><i className="fa fa-edit"></i></Link>
